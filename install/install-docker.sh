#!/usr/bin/env bash
# Isengard LEGACY docker-compose install. Brings up controller + agent on
# this host from pre-built GHCR images via docker compose. No source
# checkout required.
#
# Phase 0.8 made systemd-native install.sh the default. This script
# stays around for operators who haven't migrated. Phase 0.10+ removes
# it. New installs should run `install/install.sh` instead.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Weavers-Engineering/Isengard/next/install/install-docker.sh | sudo bash
#
