...
//! caller treats as a 1h defer).

use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};

/// Configuration for one external gate. Lives on a `Policy` row at any
/// scope; resolution merges per the standard layered rules.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ExternalGate {
    /// HTTPS URL the updater POSTs to.
    pub url: String,
    /// Optional HMAC-SHA256 signing secret. The same scheme as 12a webhooks
    /// (`X-Isengard-Signature: sha256=<hex>`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub secret: Option<String>,
    /// Per-evaluation timeout in seconds. The evaluator caps the total
    /// outbound request duration to this value; a timeout maps to
    /// [`GateDecision::Manual`] (escalate) per the spec.
...
pub struct GatePayload {
    /// Always `"update.gate"`. Lets receivers reuse one endpoint for
    /// multiple Isengard event sources.
    pub kind: String,
    pub host_id: String,
    pub stack: String,
    pub service: String,
...
/// Outcome the evaluator returns to the cycle. Distinct from
/// [`GateResponse`] because the evaluator collapses transport-level errors
/// into specific decisions per the spec.
...
impl GateDecision {
    /// Stable string for events. Matches the spec's `update.gated_<x>`
    /// suffix vocabulary.
