 ...
 *
 * Source: `design/concepts/home/v1.html` right column. Standalone fetch (not
 * tied to a specific stack like `useDeployments`) since the home view shows
 * everything in flight.
 *
 * Progress is heuristic: the controller doesn't expose a true %. blue-green
 * lifecycle states map to four bands:
 *   spinning_up  20%
 *   healthcheck  40%
 *   switching    60%
 *   draining    100%
...
function shortDigest(d: string | null): string {
  if (!d) return ''
  // sha256:abc1234... truncates to the first 7 hex chars.
...
<template>
  <div class="rounded-iso-lg border bg-iso-bg-elevated flex flex-col overflow-hidden"
    ...
    <div v-if="count === 0" class="p-4 text-iso-xs text-iso-text-faint">
      No active deploys. Rollouts appear here as they switch traffic.
    </div>
