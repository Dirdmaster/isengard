<script setup lang="ts">
import { useWizardStore } from '~/stores/wizard'

const wizard = useWizardStore()

const nextSteps = [
  {
    icon: 'lucide:server',
    title: 'Add another host',
    body: 'Build a fleet of two or more.',
    action: () => wizard.$patch({ step: 2, installCommand: null, hostId: null, enrollmentToken: null }),
  },
  {
    icon: 'lucide:bell',
    title: 'Set up notifications',
    body: 'Telegram, Discord, or webhooks.',
    action: () => navigateTo('/settings'),
  },
  {
    icon: 'lucide:book-open',
    title: 'Read the docs',
    body: 'Stack labels, update policies, fleets.',
    action: () => window.open('https://github.com/Dirdmaster/isengard', '_blank'),
  },
]

function discoverySummary(): string {
  if (wizard.discoveredStacks === 0 && wizard.discoveredServices === 0) {
    return 'Agent reporting. First container scan in progress.'
  }
  const stacksLabel = `${wizard.discoveredStacks} ${wizard.discoveredStacks === 1 ? 'stack' : 'stacks'}`
  const servicesLabel = `${wizard.discoveredServices} ${wizard.discoveredServices === 1 ? 'service' : 'services'}`
  return `Isengard discovered ${stacksLabel} · ${servicesLabel} running on this host.`
}
</script>

<template>
  <div class="flex-1 flex flex-col items-center gap-6 py-10 overflow-y-auto">
    <div class="w-24 h-24 rounded-full bg-iso-success/10 border border-iso-success flex items-center justify-center">
      <Icon name="lucide:check" class="w-12 h-12 text-iso-success" />
    </div>

    <div class="flex flex-col items-center gap-2 text-center">
      <h1 class="font-mono text-[28px] font-semibold text-iso-text-primary">
        {{ wizard.enrolledHost?.hostname ?? wizard.hostname ?? 'Host' }} is connected
      </h1>
      <p class="text-sm text-iso-text-muted">{{ discoverySummary() }}</p>
    </div>

    <div class="w-[560px] flex flex-col gap-3 mt-4">
      <p class="text-[10px] uppercase tracking-wider font-medium text-iso-text-faint">What's next</p>
      <button
        v-for="item in nextSteps"
        :key="item.title"
        class="flex items-center justify-between gap-3 px-4 py-3.5 rounded-lg bg-iso-bg-elevated border border-iso-border-subtle hover:border-iso-text-secondary text-left"
        @click="item.action"
      >
        <div class="flex items-center gap-3">
          <Icon :name="item.icon" class="w-4 h-4 text-iso-text-secondary" />
          <div class="flex flex-col gap-0.5">
            <span class="font-mono text-[13px] font-medium text-iso-text-primary">{{ item.title }}</span>
            <span class="text-[11px] text-iso-text-faint">{{ item.body }}</span>
          </div>
        </div>
        <Icon name="lucide:chevron-right" class="w-3.5 h-3.5 text-iso-text-faint" />
      </button>
    </div>
  </div>
</template>
