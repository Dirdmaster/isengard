        ...
        while let Ok(Some(msg)) = inbound.message().await {
            match msg.payload {
                ...
                Some(isengard_proto::pb::controller_message::Payload::WriteCompose(req)) => {
                    ...
                    // Track A teardown (2026-05-15): the lifecycle-hook
                    // pre/post/failure dispatch is gone. WriteCompose is
                    // now a pure compose-write op. The proto's hooks +
                    // manifest_toml fields still exist (Task 7 drops
                    // them) but the agent ignores them.
